...
/****************************************/
enum ActionType{DOWN,UP,LEFT,RIGHT};
...
bool OutofBoundMove(int index, ActionType act){
	bool flag=false;
	if( index<0 || index>8 ) flag=true;
	else if( index>=0 && index<=2 && move_list[act]=="up" ) flag=true;
	else if( (index==0 || index==3 || index==6)&& move_list[act]=="left" ) flag=true;
	else if( (index==2 || index==5 || index==8)&& move_list[act]=="right" )  flag=true;
	else if( index>=6 && index<=8 && move_list[act]=="down" ) flag=true;
	return flag;     
}
