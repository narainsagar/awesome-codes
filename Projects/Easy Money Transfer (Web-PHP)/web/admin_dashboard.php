												...
												if($result1){
													if(mysql_num_rows($result1)>0){
														while ($row = mysql_fetch_array($result1)) {
															...
															$s_query = ("select full_name from shop_keeper where mobile_no='$smobile'");
															$result_s = mysql_query($s_query, $link2) or die("Error : " .mysql_error());
															if($result_s){
																if(mysql_num_rows($result_s)>0){
																	$s_row = mysql_fetch_array($result_s);
																	$sname = $s_row['full_name'];
																}
															}
