...
{   require 'connection-bld.php' ; 
	...
	if($valid){
		...
		$query1 = ("SELECT * FROM sfo WHERE mobile_no='$login_id' and passwd='$login_passwrd'"); // query to check user  registered. 
		$result1 = mysql_query($query1, $link2) or die("query1 problem..");        // execute query
		if( $result1 ){
			 // AUTHENTICATED..!! VALID SHOPKEEPER ID and PASSWRD.
			 if(mysql_num_rows($result1) == 0) {
					$query2 = ("SELECT mobile_no FROM sfo WHERE mobile_no='$login_id'"); // query to check user  forgot passwd. 
					$result2 = mysql_query($query2, $link2) or die("query1 problem..");        // execute query
					if($result2){
						if(mysql_num_rows($result2) == 0){
							// not registered yet/invalid user..
							    $trx_success = "Sorry..!! Not a Registered SFO..";
								echo "Sorry..!! Not a Registered SFO..";
						}
						else{
			...
			//	echo "Valid SFO- Welcome..!!";
				$trx_success = "Login Successfull.!";
				$login = true;
				$row = mysql_fetch_array($result1);
				session_start();
				$_SESSION['login'] = $login;
				$_SESSION['login_type'] = "SFO";
        ...
        <div class="login">
			...
			<!--
			    <div class="col_1_of_login span_1_of_login">
					...
					<div class="button1">
					   <a href="sfo_register.php"><input type="submit" name="Submit" value="Create an Account"></a>
					 </div>
					 <div class="clear"></div>
				</div>
				-->
				...
				<div class="login-title">
	           		<h4 class="title">Registered SFO</h4>
					<div id="loginbox" class="loginbox">
					 <span class="error"> <?php echo $trx_success;?> </span>
						<form action="sfo_login.php" method="post" name="login" id="login-form">
